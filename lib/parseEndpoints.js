...
function toOpenapi(text) {
  ...
  text.split(/[\r\n]+/).forEach((line) => {
    ...
    if (_.endsWith(line, ":")) {
      const currentTagLineData = _.trim(line, ":").split(" ");
      const currentTagInternal = currentTagLineData[0] != "^public";

      currentTag = {
        name: currentTagLineData[currentTagInternal ? 0 : 1],
        "x-internal": currentTagInternal,
      };
      ...
      if (!_.find(data.tags, { name: currentTag.name })) {
    ...
    if (currentTag) {
      def.tags = [currentTag.name];
      // cascade 'x-internal' flag down to endpoints if set at the tag level
      if (currentTag["x-internal"]) {
        def = { ...def, "x-internal": true };
      }
    }
